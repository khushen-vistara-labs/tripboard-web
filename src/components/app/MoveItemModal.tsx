...
export function MoveItemModal({ item, onMove, onClose }: { item: ItineraryItem; onMove: (date: string, time: string | undefined, reason: string) => Promise<void>; onClose: () => void }) {
  ...
  return <Modal title={`Move ${item.title}`} description="The activity stays connected to its checklist, booking, and history." onClose={onClose}>
    <form className="form-stack" onSubmit={async (event) => { event.preventDefault(); setSaving(true); await onMove(date, time || undefined, reason.trim()); setSaving(false); onClose(); }}>
      ...
      <label>Reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Rain, running late, venue closed…" rows={3} required/></label>
