...
function ImportantNotes({ data }: { data: TripBoardData }) {
  const [adding, setAdding] = useState(false); const [editing, setEditing] = useState<TripNote | null>(null); const [expanded, setExpanded] = useState<string | null>(null); const [copied, setCopied] = useState<string | null>(null);
  ...
  return <><div className="guide-intro"><div><p className="eyebrow">TRIP GUIDE</p><h2>Quick reference</h2><p>Small answers for when you are already out the door.</p></div><button className="button primary" onClick={() => setAdding(true)}><Plus size={16}/> Add reference</button></div><section className="guide-board">{[...grouped.entries()].map(([category, notes]) => <section className="guide-category" key={category}><h3><span aria-hidden="true">{guideIcon(category)}</span>{category}</h3><div className="guide-list">{notes.map((note) => { const isExpanded = expanded === note.id; const canCopy = Boolean(note.copyText || copyableFor(note.body)); return <article className={isExpanded ? "expanded" : ""} key={note.id}><button className="guide-main" onClick={() => setExpanded(isExpanded ? null : note.id)} aria-expanded={isExpanded}><span className="guide-icon" aria-hidden="true">{note.icon || guideIcon(category)}</span><span><strong>{note.title}</strong><small>{note.summary || summaryFor(note.body)}</small></span><ChevronRight size={17}/></button>{isExpanded && <div className="guide-detail"><p>{note.body}</p><div className="guide-detail-actions">{canCopy && <button className="text-button" onClick={() => void copy(note)}><Copy size={14}/>{copied === note.id ? "Copied" : "Copy phrase"}</button>}<button className="text-button" onClick={() => setEditing(note)}><Pencil size={14}/> Edit</button><button className="text-button danger" onClick={() => { if (window.confirm(`Delete “${note.title}”?`)) void data.deleteNote(note.id); }}><Trash2 size={14}/> Delete</button></div></div>}</article>; })}</div></section>)}{grouped.size === 0 && <div className="panel empty-state"><StickyNote size={28}/><h3>No quick references yet</h3><p>Add a reusable phrase, payment rule, or travel essential.</p></div>}</section>{adding && <NoteModal onClose={() => setAdding(false)} onSave={async (note) => { await data.addNote(note); setAdding(false); }}/>} {editing && <NoteModal note={editing} onClose={() => setEditing(null)} onSave={async (note) => { await data.editNote(editing.id, note); setEditing(null); }}/>}</>;
}

const excludedGuideSections = ["arrival", "preparation", "strategy", "culture", "experience"];
function isGuideNote(note: TripNote) { return !excludedGuideSections.some((term) => note.section.toLowerCase().includes(term)) && !/^(after|before|on \d)/i.test(note.body.trim()); }
function guideCategory(section: string) { const value = section.toLowerCase(); if (value.includes("food") || value.includes("language")) return "Food & language"; if (value.includes("money")) return "Money"; if (value.includes("transport") || value.includes("octopus")) return "Transport"; if (value.includes("connect")) return "Connectivity"; if (value.includes("macau")) return "Macau"; return section; }
...
function NoteModal({ note, onClose, onSave }: { note?: TripNote; onClose: () => void; onSave: (note: Pick<TripNote, "section" | "title" | "body" | "summary" | "icon" | "copyText">) => Promise<void> }) {
  const [section, setSection] = useState(note?.section ?? "Essentials"); const [title, setTitle] = useState(note?.title ?? ""); const [summary, setSummary] = useState(note?.summary ?? ""); const [body, setBody] = useState(note?.body ?? ""); const [icon, setIcon] = useState(note?.icon ?? ""); const [copyText, setCopyText] = useState(note?.copyText ?? ""); const [saving, setSaving] = useState(false);
  return <Modal title={note ? "Edit quick reference" : "Add quick reference"} description="Keep it reusable and easy to scan while travelling." onClose={onClose}><form className="form-stack" onSubmit={async (event) => { event.preventDefault(); setSaving(true); await onSave({ section, title, summary, body, icon, copyText }); setSaving(false); }}><label>Category<input value={section} onChange={(event) => setSection(event.target.value)} maxLength={80} required placeholder="e.g. Money, Macau, Food & language"/></label><label>Title<input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} required placeholder="e.g. Choose HKD at the ATM"/></label><label>Short answer<input value={summary} onChange={(event) => setSummary(event.target.value)} maxLength={240} placeholder="The one-line answer shown on the guide"/></label><label>Icon <span className="optional">optional</span><input value={icon} onChange={(event) => setIcon(event.target.value)} maxLength={16} placeholder="e.g. 💳"/></label><label>More detail<textarea value={body} onChange={(event) => setBody(event.target.value)} rows={6} required placeholder="Extra context shown only when opened"/></label><label>Copyable text <span className="optional">optional</span><textarea value={copyText} onChange={(event) => setCopyText(event.target.value)} rows={2} placeholder="A phrase or number to copy quickly"/></label><div className="form-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={saving}>{saving ? "Saving…" : "Save reference"}</button></div></form></Modal>;
}
