...
function GuideAdmin({ note, onEdit, onDelete }: { note: TripNote; onEdit: (note: TripNote) => void; onDelete: () => void }) { return <details className="guide-admin"><summary aria-label="Edit guide reference">•••</summary><div><button className="text-button" onClick={() => onEdit(note)}><Pencil size={14}/> Edit</button><button className="text-button danger" onClick={() => { if (window.confirm(`Delete “${note.title}”?`)) onDelete(); }}><Trash2 size={14}/> Delete</button></div></details>; }
