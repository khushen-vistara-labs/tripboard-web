...
function ImportantNotes({ data }: { data: TripBoardData }) {
  ...
  const phrases = category("Food & language"); const phrase = phrases[0];
...
function GuideActions({ note, copied, onCopy, onEdit, onDelete }: { note: TripNote; copied: string | null; onCopy: (note: TripNote) => Promise<void>; onEdit: (note: TripNote) => void; onDelete: () => void }) { const play = () => { const text = note.copyText || copyableFor(note.body); if (!text || !("speechSynthesis" in window)) return; const speech = new SpeechSynthesisUtterance(text); speech.lang = "yue-HK"; window.speechSynthesis.cancel(); window.speechSynthesis.speak(speech); }; return <div className="guide-actions">{(note.copyText || copyableFor(note.body)) && <><button className="text-button" onClick={() => void onCopy(note)}><Copy size={14}/>{copied === note.id ? "Copied" : "Copy"}</button><button className="text-button" onClick={play}>▶ Play</button></>}<details className="guide-admin"><summary aria-label="Edit guide reference">•••</summary><div><button className="text-button" onClick={() => onEdit(note)}><Pencil size={14}/> Edit</button><button className="text-button danger" onClick={() => { if (window.confirm(`Delete “${note.title}”?`)) onDelete(); }}><Trash2 size={14}/> Delete</button></div></details></div>; }
