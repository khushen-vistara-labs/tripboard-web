...
function ImportantNotes({ data }: { data: TripBoardData }) {
  ...
  return <><div className="section-actions"><button className="button primary" onClick={() => setAdding(true)}><Plus size={16}/> Add note</button></div><section className="notes-board">{[...grouped.entries()].map(([section, notes]) => <section className="panel note-section" key={section}><div className="note-section-heading"><p className="eyebrow">IMPORTANT NOTES</p><h2>{section}</h2><p>{notes.length} {notes.length === 1 ? "note" : "notes"}</p></div><div className="note-list">{notes.map((note, index) => <article key={note.id}><div className="note-number" aria-hidden="true">{index + 1}</div><div className="note-content"><h3>{note.title}</h3><NoteBody body={note.body}/></div><div className="note-actions"><button aria-label={`Edit ${note.title}`} onClick={() => setEditing(note)}><Pencil size={15}/></button><button aria-label={`Delete ${note.title}`} onClick={() => { if (window.confirm(`Delete “${note.title}”?`)) void data.deleteNote(note.id); }}><Trash2 size={15}/></button></div></article>)}</div></section>)}{data.notes.length === 0 && <div className="panel empty-state"><StickyNote size={28}/><h3>No important notes yet</h3><p>Add shared guidance, a travel reminder, or a new category whenever you need it.</p></div>}</section>{adding && <NoteModal onClose={() => setAdding(false)} onSave={async (note) => { await data.addNote(note); setAdding(false); }}/>} {editing && <NoteModal note={editing} onClose={() => setEditing(null)} onSave={async (note) => { await data.editNote(editing.id, note); setEditing(null); }}/>}</>;
}

function NoteBody({ body }: { body: string }) {
  const points = body.split(/(?:\r?\n|[•;])+\s*/).map((point) => point.trim()).filter(Boolean);
  if (points.length < 2) return <p className="note-copy">{body}</p>;
  return <ol className="note-points">{points.map((point, index) => <li key={`${index}-${point}`}>{point}</li>)}</ol>;
}
