...
const sections: { id: MoreSection; label: string; help: string; icon: typeof Ticket }[] = [
  ...
  { id: "bookings", label: "Documents & files", help: "Bookings, travel records, and private files", icon: FileText },
...
const supportedFileTypes = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/jpeg", "image/png", "image/webp"];
const fileAccept = supportedFileTypes.join(",");
const documentTypes = [
  ["PASSPORT", "Passport"],
  ["IMMIGRATION", "Immigration / visa"],
  ["TRAVEL_DOCUMENT", "Travel document / PAR slip"],
  ["WORK_DOCUMENT", "Work or office document"],
  ["PERSONAL_DOCUMENT", "Personal document or photo"],
  ["OTHER_DOCUMENT", "Other document"],
] as const;

function isDocumentRecord(record: Pick<Booking, "type">) {
  return ["PASSPORT", "IMMIGRATION", "TRAVEL_DOCUMENT", "WORK_DOCUMENT", "PERSONAL_DOCUMENT", "OTHER_DOCUMENT"].includes(record.type);
}

function recordTypeLabel(record: Pick<Booking, "type">) {
  return isDocumentRecord(record)
    ? documentTypes.find(([value]) => value === record.type)?.[1] ?? "Document"
...
export function MoreScreen({ data, initialSection, installPrompt }: { data: TripBoardData; initialSection: MoreSection; installPrompt: InstallPromptEvent | null }) {
  ...
  const uploadBookingFile = async (bookingId: string, file: File) => {
    setMessage("");
    if (!supportedFileTypes.includes(file.type)) { setMessage("Choose a PDF, Word document, JPEG, PNG, or WebP file."); return; }
    if (file.size > 15 * 1024 * 1024) { setMessage("Files must be 15 MB or smaller."); return; }
    const supabase = getSupabaseBrowserClient();
    if (!supabase) { setMessage("Connect Supabase before uploading private files."); return; }
    ...
    if (!navigator.onLine) { await enqueueMutation({ tripId: data.trip.id, entity: "booking-file", command: "upload", payload: { id, bookingId, path, filename: safeName, mimeType: file.type, fileSize: file.size, file } }); setMessage("File saved on this device and queued for upload."); return; }
    ...
    if (uploadError) { setMessage("The file could not be uploaded. Try again."); return; }
    ...
    if (metadataError) { await supabase.storage.from("booking-documents").remove([path]); setMessage("The file could not be saved. Try again."); return; }
    setMessage("File uploaded privately."); await data.refresh();
  };

  const openTicket = async (path?: string) => {
    if (!path) { setMessage("This preview does not include the actual private file."); return; }
    const supabase = getSupabaseBrowserClient(); if (!supabase) { setMessage("Sign in to open this private file."); return; }
    const { data: signed, error } = await supabase.storage.from("booking-documents").createSignedUrl(path, 120);
    if (error) { setMessage("The private file could not be opened. Try again."); return; }
    window.open(signed.signedUrl, "_blank", "noopener,noreferrer");
  };
  const downloadFiles = async (files: { name: string; path?: string }[]) => {
    if (files.length === 0) { setMessage("Add a file before downloading."); return; }
    const paths = files.flatMap((file) => file.path ? [file] : []);
    if (paths.length === 0) { setMessage("This preview does not include an available private file."); return; }
    const supabase = getSupabaseBrowserClient(); if (!supabase) { setMessage("Sign in to download private files."); return; }
    for (const file of paths) {
      const { data: blob, error } = await supabase.storage.from("booking-documents").download(file.path!);
      if (error || !blob) { setMessage("One or more files could not be downloaded. Try again."); return; }
      const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = file.name; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    }
    setMessage(paths.length === 1 ? "Private file download started." : `${paths.length} private file downloads started.`);
  };
  const deleteBookingFile = async (file: { id?: string; path?: string }) => {
    if (!file.id || !file.path || !window.confirm("Delete this private file?")) return;
    ...
    if (storageError) { setMessage("The private file could not be deleted."); return; }
    ...
    setMessage("Private file deleted."); await data.refresh();
  };
  const replaceBookingFile = async (bookingId: string, oldFile: { id?: string; path?: string }, file: File) => {
    if (!oldFile.id || !oldFile.path) return; if (!supportedFileTypes.includes(file.type) || file.size > 15 * 1024 * 1024) { setMessage("Choose a PDF, Word document, JPEG, PNG, or WebP file up to 15 MB."); return; }
    ...
    setMessage("File replaced safely."); await data.refresh();
  };
  const deleteBookingSafely = async (booking: Booking) => {
    ...
    const supabase = getSupabaseBrowserClient(); if (!supabase) return;
    if (!navigator.onLine) { for (const file of booking.files) if (file.id && file.path) await enqueueMutation({ tripId: data.trip.id, entity: "booking-file", command: "delete", payload: { fileId: file.id, path: file.path } }); await data.deleteBooking(booking.id); setMessage("Record deletion queued until you reconnect."); return; }
    const paths = booking.files.flatMap((file) => file.path ? [file.path] : []); if (paths.length) { const { error: cleanupError } = await supabase.storage.from("booking-documents").remove(paths); if (cleanupError) { setMessage("Private files could not be cleaned up, so the record was kept."); return; } }
    await data.deleteBooking(booking.id); setMessage("Record and private files deleted.");
  };

  return <>
    <header className="screen-header"><div><p className="eyebrow">TRIP TOOLS</p><h1>{section === "overview" ? "More" : sections.find((item) => item.id === section)?.label}</h1><p>{section === "overview" ? "Documents, places, alerts, members, and settings." : sections.find((item) => item.id === section)?.help}</p></div>{section !== "overview" && <button className="button secondary" onClick={() => setSection("overview")}>Back to More</button>}</header>
    ...
    {section === "notes" && <ImportantNotes data={data}/>} {section === "bookings" && <Bookings data={data} onOpenTicket={openTicket} onDownload={downloadFiles} onUpload={uploadBookingFile} onReplaceFile={replaceBookingFile} onDeleteFile={deleteBookingFile} onDeleteBooking={deleteBookingSafely}/>}
...
function ImportantNotes({ data }: { data: TripBoardData }) {
  ...
  const guideCategories = [...new Set(data.notes.filter(isGuideNote).map((note) => guideCategory(note.section)).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  const category = (name: string) => grouped.get(name) ?? [];
  const phrases = category("Food & language");
  const additionalCategories = [...grouped.keys()].filter((name) => !["Food & language", "Emergency / Help", "After landing", "Money", "Macau", "Connectivity", "Transport", "Food"].includes(name));
  return <><div className="guide-toolbar"><button className="button secondary" onClick={() => setAdding(true)}><Plus size={16}/> Add reference</button></div><section className="field-guide">{phrases.length > 0 && <section className="guide-panel guide-phrases"><GuideHeading icon="🍜" title="Essentials & language"/><div className="phrase-grid">{phrases.map((phrase, index) => <PhraseCard key={phrase.id} note={phrase} featured={index === 0} copied={copied} onCopy={copy} onShowLarge={setLargePhrase} onEdit={setEditing} onDelete={() => void data.deleteNote(phrase.id)}/>)}</div></section>}{hotel && <section className="guide-panel guide-hotel"><GuideHeading icon="🏨" title="Get me home"/><strong>{hotel.name}</strong><address>{hotel.address}</address><div className="guide-actions"><button className="text-button" onClick={() => void copyAddress(hotel.address)}><Copy size={14}/>{copied === "hotel-address" ? "Copied" : "Copy address"}</button><button className="text-button" onClick={() => setLargeHotel(hotel)}>Show large</button>{hotel.mapsUrl && <a className="text-button" href={hotel.mapsUrl} target="_blank" rel="noreferrer"><ExternalLink size={14}/> Open map</a>}</div></section>}{category("Emergency / Help").length > 0 && <section className="guide-panel guide-checks guide-emergency"><GuideHeading icon={guideIcon("Emergency / Help")} title="Emergency / Help"/>{category("Emergency / Help").map((note) => <GuideReference key={note.id} note={note} copied={copied} onCopy={copy} onEdit={setEditing} onDelete={() => void data.deleteNote(note.id)}/>)}</section>}{category("After landing").map((note) => <section className="guide-panel guide-sequence" key={note.id}><GuideHeading icon="✈️" title="After landing at HKG"/><ol>{splitPoints(note.body).map((step, index) => <li key={step}><span>{index + 1}</span>{step}</li>)}</ol>{note.summary && <aside className="guide-warning"><strong>Airport cash</strong><p>{note.summary}</p></aside>}<GuideActions note={note} copied={copied} onCopy={copy} onEdit={setEditing} onDelete={() => void data.deleteNote(note.id)}/></section>)}{category("Money").length > 0 && <section className="guide-panel guide-money"><GuideHeading icon="💳" title="Money & payments"/><div className="guide-rules">{category("Money").map((note) => <MoneyReference key={note.id} note={note} onEdit={setEditing} onDelete={() => void data.deleteNote(note.id)}/>)}</div></section>}{["Macau", "Connectivity", "Transport", "Food"].map((name) => category(name).length ? <section className="guide-panel guide-checks" key={name}><GuideHeading icon={guideIcon(name)} title={name}/>{category(name).map((note) => <GuideReference key={note.id} note={note} copied={copied} onCopy={copy} onEdit={setEditing} onDelete={() => void data.deleteNote(note.id)}/>)}</section> : null)}{additionalCategories.map((name) => <section className="guide-panel guide-checks" key={name}><GuideHeading icon={guideIcon(name)} title={name}/>{category(name).map((note) => <GuideReference key={note.id} note={note} copied={copied} onCopy={copy} onEdit={setEditing} onDelete={() => void data.deleteNote(note.id)}/>)}</section>)}</section>{grouped.size === 0 && <div className="panel empty-state"><StickyNote size={28}/><h3>No quick references yet</h3><p>Add a reusable phrase, payment rule, or travel essential.</p></div>}{largePhrase && <Modal title={largePhrase.title} description="Show this directly to staff." onClose={() => setLargePhrase(null)}><div className="large-phrase"><strong>{largePhrase.copyText || copyableFor(largePhrase.body)}</strong><span>{friendlyPronunciation(largePhrase)}</span><p>{phraseMeaning(largePhrase)}</p></div></Modal>}{largeHotel && <Modal title="Get me home" description={largeHotel.name} onClose={() => setLargeHotel(null)}><div className="large-address"><strong>{largeHotel.address}</strong><p>{largeHotel.name}</p></div></Modal>}{adding && <NoteModal categories={guideCategories} onClose={() => setAdding(false)} onSave={async (note) => { await data.addNote(note); setAdding(false); }}/>} {editing && <NoteModal categories={guideCategories} note={editing} onClose={() => setEditing(null)} onSave={async (note) => { await data.editNote(editing.id, note); setEditing(null); }}/>}</>;
}

function GuideHeading({ icon, title }: { icon: string; title: string }) { return <h3><span aria-hidden="true">{icon}</span>{title}</h3>; }
function MoneyReference({ note, onEdit, onDelete }: { note: TripNote; onEdit: (note: TripNote) => void; onDelete: () => void }) { const isDcc = note.title.toLowerCase() === "atm"; const rules = (note.summary ?? summaryFor(note.body)).split(" · "); return <article className={isDcc ? "money-reference dcc-reference" : "money-reference"}><strong>{note.title}</strong>{isDcc ? <p className="dcc-rule"><b>Choose HKD</b><span>·</span><b>Decline INR conversion / DCC</b></p> : <ul className="money-rules">{rules.map((rule) => <li key={rule}>{rule}</li>)}</ul>}<details className="guide-detail-toggle"><summary>{isDcc ? "ATM guidance" : "Payment details"}</summary><p>{note.body}</p></details><GuideAdmin note={note} onEdit={onEdit} onDelete={onDelete}/></article>; }
function GuideReference({ note, copied, onCopy, onEdit, onDelete }: { note: TripNote; copied: string | null; onCopy: (note: TripNote) => Promise<void>; onEdit: (note: TripNote) => void; onDelete: () => void }) { return <article><strong>{note.title}</strong><ul>{referenceItems(note).map((item) => <li key={item}>{item}</li>)}</ul>{note.body && <details className="guide-detail-toggle"><summary>{detailLabel(note)}</summary><p>{note.body}</p></details>}<GuideActions note={note} copied={copied} onCopy={onCopy} onEdit={onEdit} onDelete={onDelete}/></article>; }
function PhraseCard({ note, featured, copied, onCopy, onShowLarge, onEdit, onDelete }: { note: TripNote; featured: boolean; copied: string | null; onCopy: (note: TripNote) => Promise<void>; onShowLarge: (note: TripNote) => void; onEdit: (note: TripNote) => void; onDelete: () => void }) { return <article className={`phrase-card${featured ? " featured" : ""}`}><div className="phrase-card-heading"><div><p>{note.title}</p><strong className="phrase-chinese">{note.copyText || copyableFor(note.body)}</strong></div><button className="phrase-speak" onClick={() => speakPhrase(note)} aria-label={`Play the easy pronunciation for ${note.title}`} title="Play easy pronunciation"><Volume2 size={18}/></button></div><p className="phrase-pronunciation">Say it like: {friendlyPronunciation(note)}</p><p className="phrase-meaning">Meaning: {phraseMeaning(note)}</p><div className="phrase-card-actions"><button className="text-button" onClick={() => void onCopy(note)}><Copy size={14}/>{copied === note.id ? "Copied" : "Copy"}</button><button className="text-button" onClick={() => onShowLarge(note)}>Show large</button><GuideAdmin note={note} onEdit={onEdit} onDelete={onDelete}/></div></article>; }
function GuideActions({ note, copied, onCopy, onEdit, onDelete }: { note: TripNote; copied: string | null; onCopy: (note: TripNote) => Promise<void>; onEdit: (note: TripNote) => void; onDelete: () => void }) { return <div className="guide-actions">{(note.copyText || copyableFor(note.body)) && <button className="text-button" onClick={() => void onCopy(note)}><Copy size={14}/>{copied === note.id ? "Copied" : "Copy"}</button>}<GuideAdmin note={note} onEdit={onEdit} onDelete={onDelete}/></div>; }
function GuideAdmin({ note, onEdit, onDelete }: { note: TripNote; onEdit: (note: TripNote) => void; onDelete: () => void }) {
  const menu = useRef<HTMLDetailsElement>(null);
  const close = () => { if (menu.current) menu.current.open = false; };
  useEffect(() => {
    const closeOnOutsidePress = (event: PointerEvent) => { if (menu.current?.open && event.target instanceof Node && !menu.current.contains(event.target)) close(); };
    document.addEventListener("pointerdown", closeOnOutsidePress);
    return () => document.removeEventListener("pointerdown", closeOnOutsidePress);
  }, []);
  return <details className="guide-admin" ref={menu}><summary aria-label="Edit guide reference">•••</summary><div><button className="text-button" onClick={() => { close(); onEdit(note); }}><Pencil size={14}/> Edit</button><button className="text-button danger" onClick={() => { close(); if (window.confirm(`Delete “${note.title}”?`)) onDelete(); }}><Trash2 size={14}/> Delete</button></div></details>;
}

function isGuideNote(note: TripNote) { return !note.section.toLowerCase().includes("culture") && !note.section.toLowerCase().includes("immigration") && !note.title.toLowerCase().includes("pre-trip"); }
function guideCategory(section: string) { const value = section.toLowerCase(); if (value.includes("arrival") || value.includes("after landing")) return "After landing"; if (value.includes("food") && value.includes("strategy")) return "Food"; if (value.includes("food") || value.includes("language")) return "Food & language"; if (value.includes("money")) return "Money"; if (value.includes("transport") || value.includes("octopus")) return "Transport"; if (value.includes("connect")) return "Connectivity"; if (value.includes("macau")) return "Macau"; return section; }
function guideIcon(category: string) { return ({ "Food & language": "🍜", Money: "💳", Transport: "🚇", Connectivity: "📶", Macau: "🇲🇴", "Emergency / Help": "🆘", Essentials: "🧳" } as Record<string, string>)[category] ?? "📌"; }
...
function speakPhrase(note: TripNote) {
  if (!("speechSynthesis" in window)) return;
  const chinese = note.copyText || copyableFor(note.body);
  const easyPronunciation = friendlyPronunciation(note);
...
function referenceItems(note: TripNote) { const source = note.summary || note.body; return source.includes(" · ") ? source.split(" · ").map((item) => item.trim()).filter(Boolean) : splitPoints(source); }
function detailLabel(note: TripNote) { return note.title === "Useful apps" ? "What each app is for" : note.title === "Macau day essentials" ? "Macau details" : "More detail"; }

function NoteModal({ note, categories, onClose, onSave }: { note?: TripNote; categories: string[]; onClose: () => void; onSave: (note: Pick<TripNote, "section" | "title" | "body" | "summary" | "icon" | "copyText" | "pronunciation" | "meaning">) => Promise<void> }) {
  const newCategoryValue = "__new_category__";
  const existingCategory = note ? guideCategory(note.section) : undefined;
  const availableCategories = [...new Set([...categories, existingCategory].filter((category): category is string => Boolean(category)))];
  const initialSection = existingCategory ?? "";
  const [selectedCategory, setSelectedCategory] = useState(initialSection);
  const [newCategory, setNewCategory] = useState(availableCategories.includes(initialSection) ? "" : initialSection);
  const [title, setTitle] = useState(note?.title ?? ""); const [summary, setSummary] = useState(note?.summary ?? ""); const [body, setBody] = useState(note?.body ?? ""); const [icon, setIcon] = useState(note?.icon ?? ""); const [copyText, setCopyText] = useState(note?.copyText ?? ""); const [pronunciation, setPronunciation] = useState(note?.pronunciation ?? ""); const [meaning, setMeaning] = useState(note?.meaning ?? ""); const [saving, setSaving] = useState(false);
  const section = selectedCategory === newCategoryValue ? newCategory.trim() : selectedCategory;

  return <Modal title={note ? "Edit quick reference" : "Add quick reference"} description="Keep it reusable and easy to scan while travelling." onClose={onClose}><form className="form-stack" onSubmit={async (event) => { event.preventDefault(); setSaving(true); await onSave({ section, title, summary, body, icon, copyText, pronunciation, meaning }); setSaving(false); }}><label>Category<select value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)} required>{!note && <option value="" disabled>Choose a category</option>}{availableCategories.map((category) => <option key={category} value={category}>{category}</option>)}<option value={newCategoryValue}>New category…</option></select></label>{selectedCategory === newCategoryValue && <label>New category<input value={newCategory} onChange={(event) => setNewCategory(event.target.value)} maxLength={80} required placeholder="e.g. Hotel, Emergency, Shopping"/></label>}<label>Title<input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} required placeholder="e.g. Choose HKD at the ATM"/></label><label>Short answer<input value={summary} onChange={(event) => setSummary(event.target.value)} maxLength={240} placeholder="The one-line answer shown on the guide"/></label><label>Chinese phrase <span className="optional">optional</span><input value={copyText} onChange={(event) => setCopyText(event.target.value)} placeholder="Text to show and copy"/></label><label>Easy pronunciation <span className="optional">optional</span><input value={pronunciation} onChange={(event) => setPronunciation(event.target.value)} placeholder="e.g. mmm yiu ngow yook"/></label><label>English meaning <span className="optional">optional</span><input value={meaning} onChange={(event) => setMeaning(event.target.value)} placeholder="What the phrase means"/></label><label>Icon <span className="optional">optional</span><input value={icon} onChange={(event) => setIcon(event.target.value)} maxLength={16} placeholder="e.g. 💳"/></label><label>More detail<textarea value={body} onChange={(event) => setBody(event.target.value)} rows={6} required placeholder="Extra context shown only when opened"/></label><div className="form-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={saving}>{saving ? "Saving…" : "Save reference"}</button></div></form></Modal>;
}
...
function Bookings({ data, onOpenTicket, onDownload, onUpload, onReplaceFile, onDeleteFile, onDeleteBooking }: { data: TripBoardData; onOpenTicket: (path?: string) => Promise<void>; onDownload: (files: { name: string; path?: string }[]) => Promise<void>; onUpload: (bookingId: string, file: File) => Promise<void>; onReplaceFile: (bookingId: string, oldFile: { id?: string; path?: string }, file: File) => Promise<void>; onDeleteFile: (file: { id?: string; path?: string }) => Promise<void>; onDeleteBooking: (booking: Booking) => Promise<void> }) {
  const [adding, setAdding] = useState(false); const [editing, setEditing] = useState<Booking | null>(null); const [selected, setSelected] = useState<Booking | null>(null); const [filter, setFilter] = useState<"all" | "travel" | "documents">("all"); const handledOpen = useRef(false);
  ...
  const records = data.bookings.filter((record) => {
    if (filter === "all") return true;
    return filter === "documents" ? isDocumentRecord(record) : !isDocumentRecord(record);
  });
  const addFiles = async (recordId: string, files: File[]) => { for (const file of files) await onUpload(recordId, file); };
  return <><div className="records-toolbar"><div className="record-filters" aria-label="Filter records"><button className={filter === "all" ? "active" : ""} aria-pressed={filter === "all"} onClick={() => setFilter("all")}>All</button><button className={filter === "travel" ? "active" : ""} aria-pressed={filter === "travel"} onClick={() => setFilter("travel")}>Travel bookings</button><button className={filter === "documents" ? "active" : ""} aria-pressed={filter === "documents"} onClick={() => setFilter("documents")}>Documents</button></div><button className="button primary" onClick={() => setAdding(true)}><Plus size={16}/> Add record</button></div><section className="booking-list">{records.map((booking) => { const isDocument = isDocumentRecord(booking); const hasDownloadableFiles = booking.files.some((file) => Boolean(file.path)); const Icon = isDocument ? FileText : Plane; return <article className={`panel booking-card${isDocument ? " document-card" : ""}`} key={booking.id}><div className="booking-icon"><Icon size={21}/></div><button className="booking-main booking-open" onClick={() => setSelected(booking)}><div className="booking-title"><span>{isDocument ? "DOCUMENT" : booking.type}</span><h2>{booking.title}</h2><p>{isDocument ? [booking.provider && `Issued by ${booking.provider}`, booking.travellers?.length && `For ${booking.travellers.join(", ")}`].filter(Boolean).join(" · ") || "Private file" : booking.provider || "Provider not added"}</p></div><div className="booking-details"><span><CalendarClock size={15}/>{booking.startsAt ? new Intl.DateTimeFormat("en", { dateStyle: "medium", ...(isDocument ? {} : { timeStyle: "short" }), timeZone: data.trip.timezone }).format(new Date(booking.startsAt)) : isDocument ? "No date added" : "Date to be confirmed"}</span><span><BookOpenCheck size={15}/>{isDocument ? "Document no." : "Reference"} <b>{booking.reference || (booking.files.length ? `${booking.files.length} file${booking.files.length === 1 ? "" : "s"}` : "Not added")}</b></span></div></button><div className="booking-actions"><span className={`booking-status ${isDocument ? "record-private" : `status-${booking.status.toLowerCase()}`}`}><Check size={13}/>{isDocument ? "PRIVATE" : booking.status}</span><div className="record-actions"><button className="record-icon-button" aria-label={`View ${booking.title}`} title="View details" onClick={() => setSelected(booking)}><Eye size={17}/></button><button className="record-icon-button" aria-label={`Download files for ${booking.title}`} title={hasDownloadableFiles ? "Download files" : "Add a file before downloading"} disabled={!hasDownloadableFiles} onClick={() => void onDownload(booking.files)}><Download size={17}/></button><label className="record-icon-button upload-button" aria-label={`Add file to ${booking.title}`} title="Add file"><Upload size={17}/><input aria-label={`Add file to ${booking.title}`} type="file" accept={fileAccept} onChange={(event) => { const file = event.target.files?.[0]; if (file) void onUpload(booking.id, file); event.currentTarget.value = ""; }}/></label></div></div></article>; })}{records.length === 0 && <div className="panel empty-state"><FileText size={28}/><h3>{data.bookings.length ? "No matching records" : "No documents or bookings yet"}</h3><p>{data.bookings.length ? "Try a different filter." : "Save a booking, passport, PAR slip, immigration paper, photo, or work document."}</p></div>}</section>
    {adding && <RecordFormModal onClose={() => setAdding(false)} onSave={async (booking, files) => { const id = await data.addBooking(booking); await addFiles(id, files); setAdding(false); }}/>} {editing && <RecordFormModal booking={editing} onClose={() => setEditing(null)} onSave={async (booking, files) => { await data.editBooking(editing.id, booking); await addFiles(editing.id, files); setEditing(null); }}/>} {selected && <BookingDetailModal booking={data.bookings.find((item) => item.id === selected.id) ?? selected} timezone={data.trip.timezone} onClose={() => setSelected(null)} onEdit={() => { setEditing(selected); setSelected(null); }} onCancel={async () => { await cancel(selected); setSelected(null); }} onDelete={async () => { await onDeleteBooking(selected); setSelected(null); }} onOpenTicket={onOpenTicket} onDownload={onDownload} onReplaceFile={onReplaceFile} onDeleteFile={onDeleteFile}/>}
  </>;
}

type EditableBooking = Omit<Booking, "id" | "tripId" | "files">;
function RecordFormModal({ booking, onClose, onSave }: { booking?: Booking; onClose: () => void; onSave: (booking: EditableBooking, files: File[]) => Promise<void> }) {
  const initialIsDocument = booking ? isDocumentRecord(booking) : false;
  const [recordKind, setRecordKind] = useState<"booking" | "document">(initialIsDocument ? "document" : "booking");
  const [title, setTitle] = useState(booking?.title ?? ""); const [type, setType] = useState(booking?.type ?? "OTHER"); const [provider, setProvider] = useState(booking?.provider ?? ""); const [reference, setReference] = useState(booking?.reference ?? ""); const [startsAt, setStartsAt] = useState(booking?.startsAt ? new Date(booking.startsAt).toISOString().slice(0, 16) : ""); const [location, setLocation] = useState(booking?.location ?? ""); const [travellers, setTravellers] = useState(booking?.travellers?.join(", ") ?? ""); const [amount, setAmount] = useState(booking?.amount ?? ""); const [currency, setCurrency] = useState(booking?.currency ?? "INR"); const [notes, setNotes] = useState(booking?.notes ?? ""); const [status, setStatus] = useState<Booking["status"]>(booking?.status ?? "CONFIRMED"); const [files, setFiles] = useState<File[]>([]); const [saving, setSaving] = useState(false);
  const isDocument = recordKind === "document";
  const setKind = (next: "booking" | "document") => { setRecordKind(next); if (next === "document" && !isDocumentRecord({ type })) setType("TRAVEL_DOCUMENT"); if (next === "booking" && isDocumentRecord({ type })) setType("OTHER"); };
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSaving(true); await onSave({ title: title.trim(), type, provider: provider.trim() || undefined, reference: reference.trim() || undefined, startsAt: startsAt ? new Date(startsAt).toISOString() : undefined, location: isDocument ? undefined : location.trim() || undefined, travellers: travellers.split(",").map((value) => value.trim()).filter(Boolean), amount: isDocument || !amount ? undefined : amount, currency: isDocument || !amount ? undefined : currency, notes: notes.trim() || undefined, status: isDocument ? "CONFIRMED" : status }, files); setSaving(false); };
  return <Modal title={booking ? isDocument ? "Edit document" : "Edit booking" : "Add document or booking"} description="Keep travel confirmations and the important files you may need together." onClose={onClose} wide><form className="form-stack" onSubmit={submit}><div className="record-kind-picker" role="group" aria-label="Record type"><button type="button" className={recordKind === "booking" ? "active" : ""} onClick={() => setKind("booking")}><Plane size={16}/> Travel booking</button><button type="button" className={recordKind === "document" ? "active" : ""} onClick={() => setKind("document")}><FileText size={16}/> Document / file</button></div>{isDocument ? <><label>Document name<input value={title} onChange={(event) => setTitle(event.target.value)} required placeholder="e.g. Alex's passport or PAR slip"/></label><div className="form-grid"><label>Category<select value={type} onChange={(event) => setType(event.target.value)}>{documentTypes.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Issuer or source<input value={provider} onChange={(event) => setProvider(event.target.value)} placeholder="Government, employer, airline…"/></label><label>Document number <span className="optional">optional</span><input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="Passport, visa, or employee ID number"/></label><label>Relevant date <span className="optional">expiry, issue, or travel date</span><input type="date" value={startsAt.slice(0, 10)} onChange={(event) => setStartsAt(event.target.value)}/></label></div><label>For / owner <span className="optional">comma separated</span><input value={travellers} onChange={(event) => setTravellers(event.target.value)} placeholder="Alex, Sam"/></label><div className="form-upload-field"><span>Attach files <small>PDF, Word, JPEG, PNG, or WebP · up to 15 MB each</small></span><label className="file-picker"><Upload size={16}/> Choose files<input type="file" accept={fileAccept} multiple onChange={(event) => setFiles(Array.from(event.target.files ?? []))}/></label>{files.length > 0 && <p>{files.map((file) => file.name).join(", ")}</p>}</div><label>Notes <span className="optional">optional</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} placeholder="What this document is for, where it may be needed, or a reminder…"/></label></> : <><label>Booking title<input value={title} onChange={(event) => setTitle(event.target.value)} required placeholder="e.g. Ferry to Macau"/></label><div className="form-grid"><label>Type<select value={type} onChange={(event) => setType(event.target.value)}>{["FLIGHT","HOTEL","ATTRACTION","FERRY","CRUISE","THEME_PARK","CABLE_CAR","TOUR","INSURANCE","ESIM","OTHER"].map((value) => <option key={value}>{value}</option>)}</select></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value as Booking["status"])}><option>PLACEHOLDER</option><option>CONFIRMED</option><option>USED</option><option>CANCELLED</option></select></label><label>Provider<input value={provider} onChange={(event) => setProvider(event.target.value)} placeholder="Airline, hotel, operator"/></label><label>Reference<input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="Confirmation or ticket number"/></label><label>Date and time<input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)}/></label><label>Location<input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Terminal, hotel, meeting point"/></label></div><label>Travellers <span className="optional">comma separated</span><input value={travellers} onChange={(event) => setTravellers(event.target.value)} placeholder="Alex, Sam"/></label><div className="form-grid"><label>Amount<input inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00"/></label><label>Currency<select value={currency} onChange={(event) => setCurrency(event.target.value)}><option>INR</option><option>HKD</option><option>MOP</option><option>USD</option><option>EUR</option><option>GBP</option></select></label></div><label>Notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} placeholder="Check-in instructions, baggage, cancellation terms…"/></label></>}<div className="form-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={saving}>{saving ? "Saving…" : booking ? isDocument ? "Save document" : "Save booking" : isDocument ? "Save document" : "Add booking"}</button></div></form></Modal>;
}

function BookingDetailModal({ booking, timezone, onClose, onEdit, onCancel, onDelete, onOpenTicket, onDownload, onReplaceFile, onDeleteFile }: { booking: Booking; timezone: string; onClose: () => void; onEdit: () => void; onCancel: () => Promise<void>; onDelete: () => Promise<void>; onOpenTicket: (path?: string) => Promise<void>; onDownload: (files: { name: string; path?: string }[]) => Promise<void>; onReplaceFile: (bookingId: string, oldFile: { id?: string; path?: string }, file: File) => Promise<void>; onDeleteFile: (file: { id?: string; path?: string }) => Promise<void> }) {
  const isDocument = isDocumentRecord(booking);
  const dateLabel = isDocument ? "Relevant date" : "Date and time";
  const dateValue = booking.startsAt ? new Intl.DateTimeFormat("en", { dateStyle: "full", ...(isDocument ? {} : { timeStyle: "short" }), timeZone: timezone }).format(new Date(booking.startsAt)) : isDocument ? "Not added" : "Not confirmed";
  return <Modal title={booking.title} description={`${recordTypeLabel(booking)} · ${isDocument ? "Private document" : booking.status}`} onClose={onClose} wide><div className="detail-grid"><div><span>{isDocument ? "Issuer or source" : "Provider"}</span><strong>{booking.provider || "Not added"}</strong></div><div><span>{isDocument ? "Document number" : "Reference"}</span><strong className="reference-value">{booking.reference || "Not added"}</strong></div><div><span>{dateLabel}</span><strong>{dateValue}</strong></div>{isDocument ? <div><span>For / owner</span><strong>{booking.travellers?.join(", ") || "Not added"}</strong></div> : <><div><span>Location</span><strong>{booking.location || "Not added"}</strong></div><div><span>Travellers</span><strong>{booking.travellers?.join(", ") || "Not added"}</strong></div><div><span>Amount</span><strong>{booking.amount ? `${booking.currency} ${booking.amount}` : "Not added"}</strong></div></>}</div>{booking.notes && <section className="detail-notes"><h3>Notes</h3><p>{booking.notes}</p></section>}<section className="detail-files"><h3>Files</h3>{booking.files.length === 0 ? <div className="mini-empty">No files uploaded yet.</div> : booking.files.map((file) => <article key={file.path ?? file.name}><button className="file-open" onClick={() => void onOpenTicket(file.path)}>{file.kind === "Image" || file.kind === "QR" ? <FileImage size={16}/> : <FileText size={16}/>}<span><strong>{file.name}</strong><small>Open private file</small></span><ExternalLink size={14}/></button><button className="text-button" onClick={() => void onDownload([file])}><Download size={13}/> Download</button><label className="text-button upload-button"><RefreshCw size={13}/> Replace<input aria-label={`Replace ${file.name}`} type="file" accept={fileAccept} onChange={(event) => { const replacement = event.target.files?.[0]; if (replacement) void onReplaceFile(booking.id, file, replacement); event.currentTarget.value = ""; }}/></label><button className="text-button danger" onClick={() => void onDeleteFile(file)}><Trash2 size={13}/> Delete</button></article>)}</section><div className="form-actions detail-actions"><button className="button secondary" onClick={onEdit}><Pencil size={14}/> Edit</button>{!isDocument && booking.status !== "CANCELLED" && <button className="button secondary" onClick={() => void onCancel()}><XCircle size={14}/> Cancel booking</button>}<button className="button danger" onClick={() => void onDelete()}><Trash2 size={14}/> Delete {isDocument ? "document" : "booking"}</button></div></Modal>;
}
