...
import { Bell, BellRing, BookOpenCheck, CalendarClock, Check, ChevronRight, Cloud, Copy, Download, ExternalLink, FileImage, FileText, History, LockKeyhole, LogOut, MapPin, Pencil, Plane, Plus, RefreshCw, Settings, Share2, ShieldCheck, Smartphone, StickyNote, Ticket, Trash2, Upload, UserMinus, Users, Volume2, Wifi, XCircle } from "lucide-react";
...
function Bookings({ data, onOpenTicket, onUpload, onReplaceFile, onDeleteFile, onDeleteBooking }: { data: TripBoardData; onOpenTicket: (path?: string) => Promise<void>; onUpload: (bookingId: string, file: File) => Promise<void>; onReplaceFile: (bookingId: string, oldFile: { id?: string; path?: string }, file: File) => Promise<void>; onDeleteFile: (file: { id?: string; path?: string }) => Promise<void>; onDeleteBooking: (booking: Booking) => Promise<void> }) {
  ...
  return <><div className="records-toolbar"><div className="record-filters" aria-label="Filter records"><button className={filter === "all" ? "active" : ""} aria-pressed={filter === "all"} onClick={() => setFilter("all")}>All</button><button className={filter === "travel" ? "active" : ""} aria-pressed={filter === "travel"} onClick={() => setFilter("travel")}>Travel bookings</button><button className={filter === "documents" ? "active" : ""} aria-pressed={filter === "documents"} onClick={() => setFilter("documents")}>Documents</button></div><button className="button primary" onClick={() => setAdding(true)}><Plus size={16}/> Add record</button></div><section className="booking-list">{records.map((booking) => { const isDocument = isDocumentRecord(booking); const Icon = isDocument ? FileText : Plane; return <article className={`panel booking-card${isDocument ? " document-card" : ""}`} key={booking.id}><div className="booking-icon"><Icon size={21}/></div><button className="booking-main booking-open" onClick={() => setSelected(booking)}><div className="booking-title"><span>{isDocument ? "DOCUMENT" : booking.type}</span><h2>{booking.title}</h2><p>{isDocument ? [booking.provider && `Issued by ${booking.provider}`, booking.travellers?.length && `For ${booking.travellers.join(", ")}`].filter(Boolean).join(" · ") || "Private file" : booking.provider || "Provider not added"}</p></div><div className="booking-details"><span><CalendarClock size={15}/>{booking.startsAt ? new Intl.DateTimeFormat("en", { dateStyle: "medium", ...(isDocument ? {} : { timeStyle: "short" }), timeZone: data.trip.timezone }).format(new Date(booking.startsAt)) : isDocument ? "No date added" : "Date to be confirmed"}</span><span><BookOpenCheck size={15}/>{isDocument ? "Document no." : "Reference"} <b>{booking.reference || (booking.files.length ? `${booking.files.length} file${booking.files.length === 1 ? "" : "s"}` : "Not added")}</b></span></div></button><div className="booking-actions"><span className={`booking-status ${isDocument ? "record-private" : `status-${booking.status.toLowerCase()}`}`}><Check size={13}/>{isDocument ? "PRIVATE" : booking.status}</span><button className="button secondary" onClick={() => setEditing(booking)}><Pencil size={14}/> Edit</button><label className="button secondary upload-button"><Upload size={15}/> Add file<input aria-label={`Add file to ${booking.title}`} type="file" accept={fileAccept} onChange={(event) => { const file = event.target.files?.[0]; if (file) void onUpload(booking.id, file); event.currentTarget.value = ""; }}/></label></div></article>; })}{records.length === 0 && <div className="panel empty-state"><FileText size={28}/><h3>{data.bookings.length ? "No matching records" : "No documents or bookings yet"}</h3><p>{data.bookings.length ? "Try a different filter." : "Save a booking, passport, PAR slip, immigration paper, photo, or work document."}</p></div>}</section>
