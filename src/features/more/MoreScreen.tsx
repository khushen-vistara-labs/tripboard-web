...
function NoteModal({ note, categories, onClose, onSave }: { note?: TripNote; categories: string[]; onClose: () => void; onSave: (note: Pick<TripNote, "section" | "title" | "body" | "summary" | "icon" | "copyText" | "pronunciation" | "meaning">) => Promise<void> }) {
  ...
  const availableCategories = [...new Set([...categories, note?.section].filter((category): category is string => Boolean(category)))];
  const initialSection = note?.section ?? availableCategories[0] ?? "Essentials";
