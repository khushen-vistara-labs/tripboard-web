  ...
  return <>
    ...
    <div className="money-main-grid">
      ...
      <section className="panel accounts-panel" id="accounts">
        ...
        <div className="account-list">{activeAccounts.map((account, index) => <article key={account.id}><span className={`account-icon account-${index % 4}`}>{account.accountClass === "EXTERNAL_SOURCE" ? <CreditCard size={17}/> : <Banknote size={17}/>}</span><div><strong>{account.name}</strong><small>{account.accountClass === "STORED_VALUE" ? `${account.currency} stored value` : `${account.currency} funding source`}</small></div>{account.accountClass === "STORED_VALUE" && <b>{formatMoney(ledger.balances[account.id] ?? "0", account.currency, account.currency === "MOP" ? "en-MO" : "en-HK")}</b>}<span className="account-actions"><button aria-label={`Edit ${account.name}`} onClick={() => setEditingAccount(account)}><Pencil size={14}/></button><button aria-label={`Archive ${account.name}`} onClick={() => { if (window.confirm(`Archive “${account.name}”? Its transaction history will be preserved.`)) void data.archivePaymentAccount(account.id); }}><Archive size={14}/></button></span></article>)}</div>
...
function PaymentAccountModal({ account, lockStructure = false, onClose, onSave }: { account?: PaymentAccount; lockStructure?: boolean; onClose: () => void; onSave: (account: Omit<PaymentAccount, "id" | "archivedAt">) => Promise<void> }) {
  const [name, setName] = useState(account?.name ?? ""); const [kind, setKind] = useState<PaymentAccount["accountClass"]>(account?.accountClass ?? "STORED_VALUE"); const [currency, setCurrency] = useState(account?.currency ?? "HKD"); const [openingBalance, setOpeningBalance] = useState(account?.openingBalance ?? "0"); const [saving, setSaving] = useState(false);
  return <Modal title={account ? "Edit payment account" : "Add payment account"} description="Accounts represent a real card, cash pocket, or stored-value wallet." onClose={onClose}><form className="form-stack" onSubmit={async (event) => { event.preventDefault(); setSaving(true); await onSave({ name: name.trim(), accountClass: kind, accountType: kind === "STORED_VALUE" ? "WALLET" : "CARD", currency, openingBalance }); setSaving(false); }}><label>Name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Octopus 1" required autoFocus/></label><div className="form-grid"><label>Type<select value={kind} disabled={lockStructure} onChange={(event) => setKind(event.target.value as PaymentAccount["accountClass"])}><option value="STORED_VALUE">Wallet or cash</option><option value="EXTERNAL_SOURCE">Card or bank account</option></select></label><label>Currency<select value={currency} disabled={lockStructure} onChange={(event) => setCurrency(event.target.value)}><option>INR</option><option>HKD</option><option>MOP</option></select></label></div><label>Opening balance<div className="amount-input"><span>{currency}</span><input inputMode="decimal" value={openingBalance} disabled={lockStructure} onChange={(event) => setOpeningBalance(event.target.value)} required/></div><small className="field-help">{lockStructure ? "Type, currency, and opening balance are locked because this account has history. Use Correct balance for reconciliation." : "Change this only to correct the starting value. Use Correct balance for later reconciliations."}</small></label><button className="button primary full" disabled={saving}>{saving ? "Saving…" : account ? "Save account" : "Add account"}</button></form></Modal>;
}
