...
export function MoneyScreen({ data }: { data: TripBoardData }) {
  ...
  return <>
    ...
    <section className="panel history-panel" id="history">
      ...
      <div className="ledger-list">{history.map((event) => <LedgerRow key={event.id} event={event} accounts={data.accounts} timezone={data.trip.timezone} onEdit={() => setEditingEvent(event)} onSettle={() => setSettling(event)} onVoid={() => setVoiding(event)}/>)}</div>
...
function LedgerRow({ event, accounts, timezone, onEdit, onSettle, onVoid }: { event: FinancialEvent; accounts: PaymentAccount[]; timezone: string; onEdit: () => void; onSettle: () => void; onVoid: () => void }) {
