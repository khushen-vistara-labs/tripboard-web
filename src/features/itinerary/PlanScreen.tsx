...
export function PlanScreen({ data }: { data: TripBoardData }) {
  ...
  return <>
    <header className="screen-header">
      ...
      <button aria-label="Previous day" disabled={currentIndex <= 0} onClick={() => { setExpandedDetailsId(null); setSelectedDate(dates[currentIndex - 1]); }}><ChevronLeft/></button>
      <div className="date-tabs" role="tablist" aria-label="Trip days">{dates.map((date, index) => { const parsed = DateTime.fromISO(date); return <button role="tab" aria-selected={selectedDate === date} className={selectedDate === date ? "active" : ""} key={date} onClick={() => { setExpandedDetailsId(null); setSelectedDate(date); }}><span>Day {index + 1}</span><strong>{parsed.toFormat("d")}</strong><small>{parsed.toFormat("ccc")}</small></button>; })}</div>
      <button aria-label="Next day" disabled={currentIndex < 0 || currentIndex >= dates.length - 1} onClick={() => { setExpandedDetailsId(null); setSelectedDate(dates[currentIndex + 1]); }}><ChevronRight/></button>
    </div>
...
function PlanRow({ item, detailsOpen, onDetailsToggle, dragging, canMoveUp, canMoveDown, onDragStart, onDragEnd, onDrop, onDone, onReopen, onMove, onMoveUp, onMoveDown, onSkip, onEdit, onDelete }: { item: ItineraryItem; detailsOpen: boolean; onDetailsToggle: (open: boolean) => void; dragging: boolean; canMoveUp: boolean; canMoveDown: boolean; onDragStart: (event: React.DragEvent<HTMLElement>) => void; onDragEnd: () => void; onDrop: (event: React.DragEvent<HTMLElement>) => void; onDone: () => void; onReopen: () => void; onMove: () => void; onMoveUp: () => void; onMoveDown: () => void; onSkip: () => void; onEdit: () => void; onDelete: () => void }) {
  const finished = item.status === "COMPLETED";
  const transportOptions = item.details?.transportOptions;
  const rowSummary = item.description ?? (transportOptions?.length ? undefined : item.transportInstructions) ?? `${item.type} · ${formatDuration(item.expectedDurationMinutes ?? 60)}`;
  return <article className={`plan-row${item.type === "transport" ? " transport-row" : ""} status-${item.status.toLowerCase()}${dragging ? " dragging" : ""}`} draggable onDragStart={onDragStart} onDragEnd={onDragEnd} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={onDrop}>
    ...
    <div className="plan-row-content"><div className="plan-row-title"><h3>{item.title}</h3><span className={`priority-badge ${item.priority.toLowerCase()}`}>{item.priority}</span>{item.status === "MOVED" && <span className="moved-badge">Moved</span>}</div>{rowSummary && <p>{rowSummary}</p>}<div className="plan-meta">{item.expectedDurationMinutes && <span><Clock3 size={13}/>{formatDuration(item.expectedDurationMinutes)}</span>}{item.mapsUrl && <a href={item.mapsUrl} target="_blank" rel="noreferrer"><MapPin size={13}/>Directions</a>}</div>{transportOptions?.length ? <TransportOptions options={transportOptions} recommended={item.details?.recommended}/> : null}{item.details && <TripDetails details={item.details} open={detailsOpen} onToggle={onDetailsToggle} hideTransportOptions={Boolean(transportOptions?.length)}/>}</div>
    <div className="row-actions"><button onClick={onMoveUp} disabled={!canMoveUp} aria-label={`Move ${item.title} earlier`}><ArrowUp size={15}/><span>Earlier</span></button><button onClick={onMoveDown} disabled={!canMoveDown} aria-label={`Move ${item.title} later`}><ArrowDown size={15}/><span>Later</span></button>{finished ? <button className="done-action" onClick={onReopen} aria-label={`Mark ${item.title} not done`}><RotateCcw size={16}/><span>Undo done</span></button> : <button className="done-action" onClick={onDone} aria-label={`Mark ${item.title} done`}><Check size={16}/><span>Done</span></button>}<button onClick={onMove} aria-label={`Move ${item.title} to another day`}><MoveRight size={16}/><span>Move day</span></button><button onClick={onEdit} aria-label={`Edit ${item.title}`}><Pencil size={16}/><span>Edit</span></button>{!finished && <button onClick={onSkip} aria-label={`Skip ${item.title}`}><SkipForward size={16}/><span>Skip</span></button>}<button onClick={onDelete} aria-label={`Delete ${item.title}`}><Trash2 size={16}/></button></div>
  </article>;
}
...
function TripDetails({ details, open, onToggle, hideTransportOptions = false }: { details: NonNullable<ItineraryItem["details"]>; open: boolean; onToggle: (open: boolean) => void; hideTransportOptions?: boolean }) {
  const hasDetails = Object.entries(details).some(([key, value]) => !((key === "transportOptions" || key === "recommended") && hideTransportOptions) && (Array.isArray(value) ? value.length > 0 : Boolean(value)));
  ...
  return <details className="trip-details" open={open} onToggle={(event) => onToggle(event.currentTarget.open)}><summary>Trip details</summary><div className="trip-details-grid">
    {details.quickNote && <Detail label="Remember" value={details.quickNote}/>} {details.booking && <Detail label="Booking" value={details.booking.replace("-", " ")}/>} {details.timingType && <Detail label="Timing" value={details.timingType.toLowerCase().replace("_", " ")}/>} {details.scheduleSensitive && <Detail label="Schedule" value="Time-sensitive — recheck before travel"/>} {details.farePerPerson && <Detail label="Fare / person" value={details.farePerPerson}/>} {details.fareForTwo && <Detail label="Fare / two" value={details.fareForTwo}/>} {details.attractionCost && <Detail label="Attraction" value={details.attractionCost}/>} {details.costStatus && <Detail label="Cost status" value={details.costStatus}/>} {details.payWith && <Detail label="Pay with" value={details.payWith}/>} {details.weather && <Detail label="Weather check" value={details.weather}/>} {details.crowdNote && <Detail label="Crowds / delays" value={details.crowdNote}/>} {details.operatingHours && <Detail label="Operating hours" value={details.operatingHours}/>} {details.eventStatus && <Detail label="Event status" value={details.eventStatus}/>} {details.hoursStatus && <Detail label="Hours status" value={details.hoursStatus}/>} {details.planningAssumption && <Detail label="Planning assumption" value={details.planningAssumption}/>} {details.priorityRule && <Detail label="Priority rule" value={details.priorityRule}/>} {details.fallback && <Detail label="Fallback" value={details.fallback}/>} {details.hotelReturn && <Detail label="Hotel return" value={details.hotelReturn}/>} {details.dietaryNote && <Detail label="Food safety" value={details.dietaryNote}/>} {details.foodNearby?.length && <Detail label="Nearby food" value={details.foodNearby.join(" · ")}/>} {details.carry?.length && <Detail label="Carry" value={details.carry.join(" · ")}/>}
