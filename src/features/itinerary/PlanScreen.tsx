...
import { ArrowDown, ArrowUp, CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, Filter, GripVertical, MapPin, MoveRight, Pencil, Plus, Search, SkipForward, Trash2 } from "lucide-react";
...
function PlanRow({ item, dragging, canMoveUp, canMoveDown, onDragStart, onDragEnd, onDrop, onDone, onMove, onMoveUp, onMoveDown, onSkip, onEdit, onDelete }: { item: ItineraryItem; dragging: boolean; canMoveUp: boolean; canMoveDown: boolean; onDragStart: (event: React.DragEvent<HTMLElement>) => void; onDragEnd: () => void; onDrop: (event: React.DragEvent<HTMLElement>) => void; onDone: () => void; onMove: () => void; onMoveUp: () => void; onMoveDown: () => void; onSkip: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <article className={`plan-row status-${item.status.toLowerCase()}${dragging ? " dragging" : ""}`} draggable onDragStart={onDragStart} onDragEnd={onDragEnd} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={onDrop}>
    ...
    <div className="plan-row-content"><div className="plan-row-title"><h3>{item.title}</h3><span className={`priority-badge ${item.priority.toLowerCase()}`}>{item.priority}</span>{item.status === "MOVED" && <span className="moved-badge">Moved</span>}</div><p>{item.description ?? item.transportInstructions ?? `${item.type} · ${item.expectedDurationMinutes ?? 60} min`}</p><div className="plan-meta">{item.expectedDurationMinutes && <span><Clock3 size={13}/>{item.expectedDurationMinutes} min</span>}{item.mapsUrl && <a href={item.mapsUrl} target="_blank" rel="noreferrer"><MapPin size={13}/>Directions</a>}</div>{item.details && <TripDetails details={item.details}/>}</div>
...
function TripDetails({ details }: { details: NonNullable<ItineraryItem["details"]> }) {
  ...
  return <details className="trip-details"><summary>Trip details</summary><div className="trip-details-grid">
    ...
    {details.transportOptions?.length ? <div className="detail-wide"><span>Transport choices</span>{details.transportOptions.map((option) => <p key={`${option.mode}-${option.label}`}><b>{option.mode}</b> · {option.label}{option.durationMinutes ? ` · ${option.durationMinutes} min` : ""}{option.cost ? ` · ${option.cost}` : ""}<br/>{option.instructions}</p>)}</div> : null}
  </div></details>;
}
